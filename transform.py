...
def compare_length(amount):
    ...
    for keyword in KEYWORDS:
        file_path = f'tweet-harvest/tweets-data/{keyword}.csv'
    ...
    for keyword in KEYWORDS:
        ...
        file_path_subset = f'tweet-harvest/tweets-data/{keyword}_transform.csv'
        df_subset.to_csv(file_path_subset, sep=';', encoding='utf-8', index=False, mode='w')


def trim_field():
  for keyword in KEYWORDS:
    file_path = f'tweet-harvest/tweets-data/{keyword}_transform.csv'
    ...
    try:
      ...
      specific_fields.to_csv(f'tweet-harvest/tweets-data/{keyword}_transform.csv', index=False, sep=';')
